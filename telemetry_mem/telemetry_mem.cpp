...
#define register_channel(name, index, type, field) version_params->register_for_channel(SCS_TELEMETRY_##name, index, SCS_VALUE_TYPE_##type, SCS_TELEMETRY_CHANNEL_FLAG_no_value, telemetry_store_##type, &shared_memory->field);
	...
	register_channel(TRUCK_CHANNEL_oil_pressure, SCS_U32_NIL,float, oil_pressure);
	register_channel(TRUCK_CHANNEL_oil_temperature, SCS_U32_NIL, float, oil_temp);
	register_channel(TRUCK_CHANNEL_water_temperature, SCS_U32_NIL, float, water_temp);
	register_channel(TRUCK_CHANNEL_odometer, SCS_U32_NIL, float, odometer);
	register_channel(TRUCK_CHANNEL_fuel, SCS_U32_NIL, float, fuel);
	register_channel(TRUCK_CHANNEL_fuel_average_consumption, SCS_U32_NIL, float, fuel_average_consum);
